...
from adamacs.ingest.bpod_new import Bpodfile_new as Bpodfile
import warnings
import pathlib
import re
import pdb
...
_AUX_SETUP_ALIASES = {
    ...
    "bench2p_SP": {"bench2p_sp", "bench2p_SP"},
    "macroscope": {"macroscope"},
    "behavior_box": {"behavior_box"},
}


def _normalize_aux_setup_type(aux_setup_type):
    ...
    if "openfield" in lower:
        return "openfield"
    for canonical, aliases in _AUX_SETUP_ALIASES.items():
        if lower in {alias.lower() for alias in aliases}:
            return canonical
    return normalized


def normalize_aux_setup_type(aux_setup_type):
    """Public normalization helper for AUX setup strings."""
    return _normalize_aux_setup_type(aux_setup_type)


def _load_stim_event_list_simple(curr_path, scan_basenames, pattern):
    for k in scan_basenames:
        stim_file_paths = [fp.as_posix() for fp in curr_path.glob(pattern)]
...
def _parse_double_paren_log(file_path, gate_on=None, gate_off=None, require_gate_on=False):
    extracted = []
    pattern = re.compile(r'\(\((.*?)\)\)')
    acquiring_on = not require_gate_on
    with open(file_path, 'r') as file:
        next(file)
        for line in file:
            if gate_on and gate_on in line:
                acquiring_on = True
                continue
            if gate_off and gate_off in line:
                acquiring_on = False
                continue
            if not acquiring_on:
                continue
            ...
            for match in matches:
                ...
                extracted.append(modified_string)
    return extracted


def _load_stim_event_list_double_paren(curr_path, scan_basenames, pattern,
                                       gate_on=None, gate_off=None, require_gate_on=False):
    for k in scan_basenames:
        stim_file_paths = [fp.as_posix() for fp in curr_path.glob(pattern)]
        if len(stim_file_paths) != 1:
            print(f"More or less than 1 stim_files found in {k} - not extracting stim IDs")
            return []
        extracted = _parse_double_paren_log(
            stim_file_paths[0],
            gate_on=gate_on,
            gate_off=gate_off,
            require_gate_on=require_gate_on,
        )
        df = pd.DataFrame(extracted, columns=['Value'])
        return df['Value']
    return []


def _load_stim_event_list_sp(curr_path, scan_basenames, pattern):
    for k in scan_basenames:
        stim_file_paths = [fp.as_posix() for fp in curr_path.glob(pattern)]
        if len(stim_file_paths) != 1:
            print(f"More or less than 1 stim_files found in {k} - not extracting stim IDs")
...
def _append_stim_events(event_types, vis_stim_event_list, timestamps):
    stim_events = list(vis_stim_event_list) if vis_stim_event_list is not None else []
    if not stim_events:
        return
    j = 0
    ...
    for stim_event in stim_events:
        event_types[stim_event] = np.append(event_types[stim_event], timestamps[j:j + 2])
        j += 2
    if len(stim_events) != np.asarray(timestamps).size / 2:
        print('Aux-File und StimLog have not the same number of stimulus onsets! CHECK THAT!')


def _insert_event_types(event_types, session_key, scan_key):
...
def ingest_bpod(sessi, scansi, root_paths=get_imaging_root_data_dir(), aux_setup_type="openfield",
    ...
    normalized = _normalize_aux_setup_type(aux_setup_type)
    return bpod_object.ingest_new(
        sessi, scansi,
        aux_setup_type=normalized,
...
def ingest_aux(session_key, scan_key, root_paths=get_imaging_root_data_dir(), aux_setup_type = "openfield",
    ...
    if not verbose:
        warnings.filterwarnings('ignore')
    aux_setup_type = _normalize_aux_setup_type(aux_setup_type)
    ...
    for curr_aux in aux_files:
        ...
        if aux_setup_type == "openfield":
            ...
            # LOAD STIMINFO
            vis_stim_event_list = _load_stim_event_list_simple(
                curr_path, scan_basenames, '*bonsai_stimulus_events*.csv'
            )
            ...
            _append_stim_events(event_types, vis_stim_event_list, ts_bonsai_vis)
            ...
            #             index += len(value)
                ...
                # raise ValueError(f"Aux-File und StimLog have not the same number of stimulus onsets!{k}")
                    ...
                    # LOAD STIMINFO
                    vis_stim_event_list = _load_stim_event_list_double_paren(
                        curr_path,
                        scan_basenames,
                        '*bonsai_stimulus_log*.csv',
                        gate_on="Master_Trigger:ON",
                        gate_off="Master_Trigger:OFF",
                        require_gate_on=True,
                    )
                    ...
                    _append_stim_events(event_types, vis_stim_event_list, ts_trial)
                   
        elif aux_setup_type in ("bench2p_oddball", "bench2p_Oddball_V2"): #TR23 - HEADFIXED Bench2p - #bench2p - needs to be set in scan schema! Taken from userfunction_consolidate_files argument
                    ...
                    _append_stim_events(event_types, vis_stim_event_list, ts_stim_vis)
                    ...
                    # LOAD STIMINFO
                    vis_stim_event_list = _load_stim_event_list_sp(
                        curr_path, scan_basenames, '*bonsai_stimulus_log*.csv'
                    )
        ...
        _insert_event_types(event_types, session_key, scan_key)
...
def fix_ocr_frames(frame_indices, L_max=3, verbose=True, log_path=None):
    ...
    Correction steps:
      1. Replace leading NaNs with 0 and protect first valid frame
    ...
    def log(correction_type, eye_frame_idx, original_value, corrected_value, diff_before=None, diff_after=None, details=""):
        """Add entry to log"""
        entry = {
        ...
        if verbose:
            print(f"[{correction_type}] frame {eye_frame_idx}: {original_value} -> {corrected_value} | {details}")
    
    def expected_from_local(valid_diffs):
        """
        Infer expected next diff from LOCAL pattern history.
        
        Key insight: Instead of tracking global position in [5,5,5,5,4] pattern,
        look at actual recent diffs. If we've seen 4 consecutive 5s, expect a 4.
        This self-corrects after disruptions rather than staying out of phase.
        """
        tail = valid_diffs[-4:]
    ...
    n = len(frames)
    if n == 0:
        return [], []  # Return empty corrected list and empty log entries

    # ---------- Step 1: Replace leading NaNs ----------
    first_valid = 0
    while first_valid < n and (not np.isfinite(frames[first_valid]) or frames[first_valid] == 0):
        first_valid += 1

    initial_nan_count = first_valid
    if initial_nan_count > 0:
        first_valid_val = int(frames[first_valid]) if first_valid < n else 0
        for idx in range(initial_nan_count):
            log('LEADING_ZERO', idx, frame_indices_int[idx], 0, 
                details=f"frame[{idx}]={frame_indices_int[idx]}->0, first_valid@{first_valid}={first_valid_val}, gap={first_valid}frames")
        frames[:initial_nan_count] = 0
    ...
    # ---------- Step 2: Detect repeated sequences (length 2 or 3) ----------
            ...
            if seq in seen:
                ...
                if not any(skip_fix[start:start+L]):
                    ...
                    for idx in range(start, start+L):
                        pos_in_seq = idx - start
                        log('DUPLICATE_SEQ', idx, int(frames[idx]), 0, 
                            details=f"seq[{pos_in_seq}]={int(frames[idx])}->0, L={L}, first@{start}, dup@{i}, gap={gap_frames}frames")
                    frames[start:start+L] = 0
                    skip_fix[start:start+L] = True
            else:
                seen[seq] = i

    # ---------- Step 3: Fix single-frame repeats using LOCAL 5/4 pattern inference ----------
    valid_diffs = []
    i = first_valid
    ...
    while i < n - 1:
        ...
        diff = None if not np.isfinite(prev) or not np.isfinite(curr) else curr - prev

        # Handle special 4,4,6 → 4,5,5
        if diff == 6 and len(valid_diffs) >= 2 and valid_diffs[-2:] == [4,4]:
            if (
                ...
                prev_frame_val = int(frames[i-1])
                log('PATTERN_446', i, int(old_i), int(frames[i]), 
                    diff_before=4, diff_after=5, 
                    details=f"frame[{i}]:{int(old_i)}->{int(frames[i])}, prev[{i-1}]={prev_frame_val}, diffs:4,4,6->4,5,5")
                log('PATTERN_446', i+1, int(old_i1), int(frames[i+1]), 
                    diff_before=6, diff_after=5, 
                    details=f"frame[{i+1}]:{int(old_i1)}->{int(frames[i+1])}, prev[{i}]={int(frames[i])}, diffs:4,4,6->4,5,5")
            i += 1
            continue
        ...
        # Fix bad diff only when next diff is invalid too
        next_bad = False
        if i + 2 < n and not skip_fix[i+1]:
            next_prev, next_curr = frames[i+1], frames[i+2]
            if np.isfinite(next_prev) and np.isfinite(next_curr):
                next_diff = next_curr - next_prev
                if next_diff not in (4, 5):
                    next_bad = True
            else:
                next_bad = True
        else:
        ...
        if next_bad:
            ...
            bad_diff_val = int(old_val - prev) if np.isfinite(old_val - prev) else None
            log('BAD_DIFF', i+1, int(old_val), int(frames[i+1]), 
                diff_before=bad_diff_val, 
                diff_after=expected,
                details=f"frame[{i+1}]:{int(old_val)}->{int(frames[i+1])}, prev[{i}]={int(prev)}, diff:{bad_diff_val}->{expected}, local_infer")

        i += 1

    # Interpolate any remaining zeros to ensure every frame has a unique index
    corrected, interp_log = interpolate_frame_indices(frames.astype(int).tolist(), avg_step=4.8, verbose=verbose)

    # ---------- Write log file ----------
    if log_path is not None:
        log_path = Path(log_path)
        
        # Combine OCR correction log entries with interpolation log entries
        all_entries = log_entries + interp_log
        
        # Write CSV log
        with open(log_path, 'w') as f:
            # Header with metadata
            f.write(f"# Frame Correction Log (OCR-based)\n")
            f.write(f"# Generated: {datetime.now().isoformat()}\n")
            f.write(f"# Total frames: {n}\n")
            f.write(f"# OCR corrections: {len(log_entries)}\n")
            f.write(f"# Interpolated frames: {len(interp_log)}\n")
            f.write(f"# L_max (duplicate detection): {L_max}\n")
            f.write(f"#\n")
            
            # CSV header
            f.write("eye_frame_idx,correction_type,original_value,corrected_value,diff_before,diff_after,details\n")
            
            # Data rows
            for entry in all_entries:
                row = [
                    str(entry['eye_frame_idx']),
                    entry['correction_type'],
                    str(entry['original_value']),
                    str(entry['corrected_value']),
                    str(entry['diff_before']) if entry['diff_before'] is not None else '',
                    str(entry['diff_after']) if entry['diff_after'] is not None else '',
                    f'"{entry["details"]}"'
                ]
                f.write(','.join(row) + '\n')
        ...
        if verbose:
            ...
            print(f"   OCR corrections: {len(log_entries)}, Interpolated: {len(interp_log)}")

    # Return both corrected indices and combined log entries for database insertion
    all_entries = log_entries + interp_log
    return corrected, all_entries
...
def ingest_egocams_ocr(session_key, scan_key, eye_cameras=['mini2p1_eye_left', 'mini2p1_eye_right'],
    ...
    for video_info in video_files:
        ...
        if 'error' not in result:
            ...
            if str(file_path).lower() in worldcam_paths:
...
def ingest_egocams_ocr_linear(session_key, scan_key, eye_cameras=['mini2p1_eye_left', 'mini2p1_eye_right'],
                               model_path=None, verbose=True, write_log=True,
                               residual_threshold=0.005, drop_trailing_invalid=True,
                               prefilter_ocr=True, prefilter_min_step=2.0,
                               prefilter_max_step=10):
    """
...
def ingest_topcam(session_key, scan_key, verbose=False):
    """
    Ingest top camera synchronization data as events in the ADAMACS pipeline.
    
    This function:
    1. Extracts OptiTrack event timestamps from database
    2. Loads top camera video file and CSV metadata  
    3. Validates frame count consistency between video and CSV
    4. Synchronizes top camera frames with OptiTrack timing by subsampling
    ...
    if verbose:
        print(f"🎥 Starting top camera ingestion for session {session_key}, scan {scan_key}")
    ...
    try:
        # Step 2: Get top camera video file and CSV metadata
        if verbose:
            print("🎥 Loading top camera data...")
            
        top_camera_files = (model.VideoRecordingNew * model.VideoRecordingNew.File & 
                          session_key & scan_key & 
                          'camera="mini2p1_top"').fetch()
        
        if len(top_camera_files) == 0:
            return {'success': False, 'error': 'No top camera video found in database'}
            
        video_path = Path(top_camera_files[0][-1])  # Last column should be file_path
        
        if verbose:
            print(f"📂 Video file: {video_path.name}")
            
        # Look for CSV file in the same directory
        csv_files = list(video_path.parent.glob("*top*.csv"))
        if not csv_files:
            csv_files = list(video_path.parent.glob("*.csv"))
            
        if not csv_files:
            return {'success': False, 'error': 'No CSV file found for top camera'}
            
        csv_path = csv_files[0]
        csv_data = pd.read_csv(csv_path)
        
        if verbose:
            print(f"📊 CSV file: {csv_path.name} ({len(csv_data):,} entries)")
    ...
    try:
        # Step 4: Synchronize top camera with OptiTrack frames
        if verbose:
            print("🎯 Synchronizing with OptiTrack timing...")
            
        # Calculate sampling ratio
        sampling_ratio = opti_fps / expected_fps
        step_size = int(round(sampling_ratio))
        ...
        top_camera_timestamps = optitrack_events[camera_synced_indices]
        
        # Calculate synchronization quality
        camera_intervals = np.diff(top_camera_timestamps)
        actual_fps = 1.0 / np.mean(camera_intervals)
        sync_quality = abs(actual_fps - expected_fps) / expected_fps * 100
        
        if verbose:
            print(f"🎬 Synchronized {len(top_camera_timestamps):,} frames")
    ...
    try:
        ...
        # Prepare event data
        ts_start = top_camera_timestamps
        ts_end = top_camera_timestamps + 0.001  # 1ms duration
        event_type = "mini2p1_top_frames"
        ...
        event.EventType.insert1({
            'event_type': event_type,
            'event_type_description': f'Synchronized mini2p1_top camera frames from OptiTrack timing at {expected_fps:.1f} Hz'
        }, skip_duplicates=True)
        ...
        return {
            ...
            'optitrack_frames_used': len(top_camera_timestamps), 
            'optitrack_frames_discarded': len(optitrack_events) - len(top_camera_timestamps),
...
def extract_csv_timestamps(csv_path, verbose=True):
    """
...
def extract_topcam_csv_timestamps(csv_path, verbose=True):
    """
    Extract timestamps from top camera Bonsai CSV files.

    The topcam CSV commonly includes an ISO 8601 timestamp column
    (often the first column). This function detects the ISO column,
    parses timestamps, and returns relative/absolute time arrays.
    """
    from dateutil import parser as date_parser
    ...
    if verbose:
        print(f"📄 Loading topcam CSV timestamps: {csv_path.name}")
    ...
    try:
        ...
        if best_col is None or best_ratio < 0.3:
            return {'error': 'No ISO 8601 timestamp column detected in topcam CSV'}

        timestamps_iso = df[best_col].astype(str).str.strip().tolist()
        ...
        # Skip leading non-timestamp rows (e.g., header)
...
def find_topcam_csv_timestamp_file(session_dir, verbose=True):
    """
    Find the Bonsai topcam timestamp CSV file for a given session directory.
    ...
    patterns = [
        '*_mini2p1_top_video_timestamps*.csv',
        '*_top_video_timestamps*.csv',
        '*top_video_timestamps*.csv',
        '*mini2p1_top*timestamps*.csv',
        '*top*timestamps*.csv'
    ]
    ...
    for pattern in patterns:
        ...
        if matches:
            if len(matches) > 1 and verbose:
                print(f"   ⚠️ Multiple topcam CSV files found, using: {matches[0].name}")
            return matches[0]

    if verbose:
        print("   ⚠️ No topcam CSV timestamp file found")
...
def ingest_topcam_csv_timestamps(session_key, scan_key, camera_type='mini2p1_top', verbose=True):
    """
    Ingest top camera CSV timestamps into the CameraTimestamps table (no OCR).
    ...
    if verbose:
        print(f"📊 Starting topcam CSV ingestion for session {session_key}, scan {scan_key}")
    ...
    for search_dir in search_dirs:
        csv_file = find_topcam_csv_timestamp_file(search_dir, verbose=verbose)
        if csv_file is not None:
            break

    if csv_file is None:
        return {'error': 'No topcam CSV timestamp file found'}

    csv_result = extract_topcam_csv_timestamps(csv_file, verbose=False)
    if 'error' in csv_result:
        return {'error': f"Topcam CSV extraction failed: {csv_result['error']}"}
...
def ingest_bpod_new(*args, **kwargs):
    return ingest_bpod(*args, **kwargs)


def ingest_aux_new(*args, **kwargs):
    return ingest_aux(*args, **kwargs)


def summarize_raw_bpod_events_new(*args, **kwargs):
    return summarize_raw_bpod_events(*args, **kwargs)


def summarize_bpod_metadata_new(*args, **kwargs):
    return summarize_bpod_metadata(*args, **kwargs)


def ingest_egocams_ocr_new(*args, **kwargs):
    return ingest_egocams_ocr(*args, **kwargs)


def ingest_egocams_ocr_linear_new(*args, **kwargs):
    return ingest_egocams_ocr_linear(*args, **kwargs)


def ingest_egocams_new(*args, **kwargs):
    return ingest_egocams(*args, **kwargs)


def ingest_topcam_new(*args, **kwargs):
    return ingest_topcam(*args, **kwargs)


def ingest_camera_timestamps_new(*args, **kwargs):
    return ingest_camera_timestamps(*args, **kwargs)


def ingest_topcam_csv_timestamps_new(*args, **kwargs):
    return ingest_topcam_csv_timestamps(*args, **kwargs)
