...
class Bpodfile(object):
    ...
    def _insert_stim_diode_trial_events(self, session_id, scan_id):
        scan_key = {"session_id": session_id, "scan_id": scan_id}
        stim_times = (event.Event & scan_key & 'event_type="stim_diode"').fetch(
            "event_start_time"
        )
        if not len(stim_times):
            return

        trial_ids, trial_starts, trial_stops = (
            trial.Trial & scan_key
        ).fetch("trial_id", "trial_start_time", "trial_stop_time", order_by="trial_id")
        if not len(trial_ids):
            return

        stim_times = np.asarray(stim_times, dtype=float)
        keys = []
        for trial_id, start_time, stop_time in zip(
            trial_ids, trial_starts, trial_stops
        ):
            in_trial = stim_times[
                (stim_times >= float(start_time)) & (stim_times <= float(stop_time))
            ]
            for stim_time in in_trial:
                keys.append(
                    {
                        "session_id": session_id,
                        "scan_id": scan_id,
                        "trial_id": trial_id,
                        "event_type": "stim_diode",
                        "event_start_time": float(stim_time),
                    }
                )
        if keys:
            trial.TrialEvent.insert(
                keys, allow_direct_insert=True, skip_duplicates=True
            )
    ...
    def ingest(self, session_id, scan_id, prompt=False,
        ...
        trial_attributes_keys = [
            ...
            if self.trial(n).attributes[attrib] is not None and (
                not hasattr(self.trial(n).attributes[attrib], '__len__') or 
                len(self.trial(n).attributes[attrib]) > 0
            )
        ]
        raw_event_keys = []
        raw_event_types = set()
    ...
    def ingest_behavior_box(self, session_id, scan_id, prompt=False, bood_trial_offset = 0,
        ...
        behavior_recording_key = {
            ...
            "recording_notes": f"BPod version: {bpod_version}, Behavior Box setup",
        }
        behavior_recording_fp_key = {
            "session_id": session_id,
            "scan_id": scan_id,
        ...
        with session.Session.connection.transaction:
            ...
            trial.TrialEvent.insert(event_keys, allow_direct_insert=True, skip_duplicates=True)      
            # self._insert_stim_diode_trial_events(session_id, scan_id)
