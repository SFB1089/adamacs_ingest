...
class Auxfile(object):
    ...
    def main_track_gate(self, gatechannel=4, channels=5):
        """Main track gate start time"""
        if not self._gate:
            digital_channels = self._digital(channels=channels)
            if len(digital_channels) > gatechannel:
                self._gate = get_timestamps(digital_channels[gatechannel], self._sample_rate)
            else:
                raise IndexError(f"Requested gatechannel {gatechannel} but only {len(digital_channels)} channels available.")
        return self._gate

    # @property
    def shutter_timestamps(self, shutterchannel = 3):
        """Laser shutter"""
        if not any(self._shutter):
            self._shutter = get_timestamps(self._digital()[shutterchannel], self._sample_rate)
        return self._shutter

    # @property
    def mini2p_channels(self, framechannel=3, linechannel=2, volumechannel=1):
        """Dict of arrays for frame, line and volume"""
        if not any(self._2p):
            self._2p = {  # vol and frame are the same in sample data
                "frame": get_timestamps(self._digital()[framechannel], self._sample_rate),
                "line": get_timestamps(self._digital()[linechannel], self._sample_rate),
                "volume": get_timestamps(self._digital()[volumechannel], self._sample_rate),
            }
        return self._2p

    # @property
    def cam_trigger(self, camchannel=0):
        """Tracking camera trigger"""
        if not any(self._cam):
            self._cam = get_timestamps(
                self._raw[self._sweep]["analogScans"][camchannel], self._sample_rate
            )
        return self._cam

    # @property
    def bpod_channels(self, trialchannel=1, rewardchannel=2, tonechannel=3, 
                      oddball_visstimchannel=0, lineartrack_trialchannel=4):
        ...
        if not self._bpod:
            self._bpod = {
                "trial": get_timestamps(
                    self._raw[self._sweep]["analogScans"][trialchannel], self._sample_rate
                ),
                "reward": get_timestamps(
                    self._raw[self._sweep]["analogScans"][rewardchannel], self._sample_rate
                ),
                "tone": get_timestamps(  # approx eq trial times with reqard times
                    self._raw[self._sweep]["analogScans"][tonechannel], self._sample_rate
                ),
                "oddball_visstim": get_timestamps( 
                    self._raw[self._sweep]["analogScans"][oddball_visstimchannel]*-1+9, self._sample_rate,
                    threshold=2.5
                ),
                "oddball_visstim2": get_timestamps( 
                    self._raw[self._sweep]["analogScans"][oddball_visstimchannel]*-1+9, self._sample_rate,
                    threshold=2.5
                ),
                # "lineartrack_rewardzone": get_timestamps( 
                #     self._raw[self._sweep]["analogScans"][0]*-1, self._sample_rate,
                #     threshold=2.5
                # ),
                # "lineartrack_licks": get_timestamps( 
                #     self._raw[self._sweep]["analogScans"][3], self._sample_rate,
                #     threshold=2.5
                # ),
                # "lineartrack_trial": get_timestamps( 
                #     self._raw[self._sweep]["analogScans"][4], self._sample_rate,
                #     threshold=2.5
                # ),
                
            }
            # Check if the index 4 exists in analogScans
            if len(self._raw[self._sweep]["analogScans"]) > 4:
                self._bpod["lineartrack_trial"] = get_timestamps( 
                    self._raw[self._sweep]["analogScans"][lineartrack_trialchannel], self._sample_rate,
                    threshold=2.5
                )
            else:
                # Handle the case where the index 4 is not present
                self._bpod["lineartrack_trial"] = None  # or any other appropriate default action
                # print("Index 4 not found in analogScans.")
        return self._bpod
