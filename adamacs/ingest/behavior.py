...
#
# Everything OCR-related here is deliberately left unchanged. adamacs carries a
# different implementation -- candidate configs probed per video, template matching
# against stored glyph exemplars -- and that is what produced the eye-camera
# timestamps already in the database, so reconstructing those has to be done from
# adamacs. New ingestion runs from here.
#
...
# ---------------------------------------------------------------------------------
def _eye_ocr_config(side, is_high_res, video_path):
# Shan: video_path added here
    if side == "left":
        if is_high_res:
