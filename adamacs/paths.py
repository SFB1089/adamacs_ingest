...
def get_rootuser_dlc_output_dir(dlc_scan_key=None):
    """Deprecated helper retained for compatibility."""
    if dlc_scan_key is None:
        return None
    from .pipeline import model
    return model.PoseEstimationTask.infer_output_dir(dlc_scan_key)

def get_user_from_dlc_root_data_dir():
    dlc_root_user = get_experiment_root_data_dir()[0].split(os.path.sep)[-1]
    if not dlc_root_user: #TR23: added empty
        dlc_root_user = get_experiment_root_data_dir()[0].split(os.path.sep)[-2]
        # raise ValueError('Could not infer user from dlc_root_data_dir - trying other apporach')
    return dlc_root_user
